...
contract AMSSimulation {
    ...
    event PartyConstituencyResults(bytes32 party, uint constitunecySeats);
    ...
    /// *** Voter is able to cast votes of any size ***
    /// *** Had to rework function due to Solidity stack limit ***
    /// @notice Cast a vote
    ...
    function castVote(bytes32 candidateName, bytes32 partyName, uint sizeOfCandidateVote, uint sizeOfPartyVote)
        external
        electionHasStarted
        electionHasNotEnded
        ...
        returns (bool)
    {
        // Add the voter's vote for their chosen candidate
        constituencies[constituencyIndexs[voters[voterIndexs[msg.sender]].constituency]]
            .candidates[constituencies[constituencyIndexs[voters[voterIndexs[msg.sender]].constituency]]
            .candidateIndexs[candidateName]].votes += sizeOfCandidateVote;

        // Record the voter's vote for their chosen party
        electionParties[partyIndexs[partyName]].votes += sizeOfPartyVote;

        // Record that a vote has been cast in the total number of votes
        totalVotes += sizeOfCandidateVote;

        // Record that the voter has now voted
        voters[voterIndexs[msg.sender]].voted = true;
