...
describe("Additional Member System Amend - Cast Vote", function () {
    ...
    it("Voter can ammend their vote", async function () {
        // Candidate the voter will vote for
        let voteCandidate = ethers.encodeBytes32String("Test Candidate1");

        // Party the voter will vote for
        let voteParty = ethers.encodeBytes32String("Test Party2");
        ...
        // Party the voter will vote for
        voteParty = ethers.encodeBytes32String("Test Party1");

        // Ammend the voters vote
        await expect(election.connect(voter).castVote(voteCandidate, voteParty))
...
describe("Additional Member System Amend - Calculate Election Results", function () {
    ...
    it("Should correctly calculate the election winner - Wikipedia AMS Example", async function () {
        ...
        expect(eventLog[76].fragment.name).to.equal("AllConstituencyWinnersCalculated");
        ...
        expect(eventLog[77].args[1]).to.equal(23);

        // Additional Seats Allocated Event
        expect(eventLog[78].fragment.name).to.equal("AdditionalSeatsAllocated");
        expect(eventLog[78].args[0]).to.equal(party3);
        expect(eventLog[78].args[1]).to.equal(7);

        // All Additional Seats Allocated Event
        expect(eventLog[79].fragment.name).to.equal("AllAdditionalSeatsAllocated");

        // Election Results Calculated Event
        expect(eventLog[80].fragment.name).to.equal("ElectionResultsCalculated");

        // Party Results Event
        expect(eventLog[81].fragment.name).to.equal("PartyResults");
        expect(eventLog[81].args[0]).to.equal(party1);
        expect(eventLog[81].args[1]).to.equal(54);

        // Party Results Event
        expect(eventLog[82].fragment.name).to.equal("PartyResults");
        expect(eventLog[82].args[0]).to.equal(party2);
        expect(eventLog[82].args[1]).to.equal(34);

        // Party Results Event
        expect(eventLog[83].fragment.name).to.equal("PartyResults");
        expect(eventLog[83].args[0]).to.equal(party3);
        expect(eventLog[83].args[1]).to.equal(7);

        // Party Results Event
        expect(eventLog[84].fragment.name).to.equal("PartyResults");
        expect(eventLog[84].args[0]).to.equal(party4);
        expect(eventLog[84].args[1]).to.equal(5);

        // Election Winner
        expect(eventLog[85].fragment.name).to.equal("ElectionWinner");
        expect(eventLog[85].args[0]).to.equal(party1);
        expect(eventLog[85].args[1]).to.equal(54);
    })
})
...
describe("Additional Member System Amend - Calculate Election Results with amended votes", function () {
    it("Should correctly calculate the election winner - Wikipedia AMS Example with ammended votes", async function () {
        ...
        // in each of the following constituencies 1st - 53rd and 36 votes in
        // the 54th constituency
        // Cast the wrong votes then ammend them
        let conservativeIndex = 0;
        for (let i = 0; i < 53; i++) {
            for (let j = 0; j < 5; j++) {
                const voterWallet = conservativeVoters[conservativeIndex].connect(ethers.provider);
