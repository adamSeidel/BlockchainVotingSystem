...
/// @title First Past the Post election contract with the ability to ammend a vote
...
contract FPTPAmend {
    ...
    struct Voter {
        ...
        bool voted;
        // Name of the candidate the voter has voted for
        bytes32 vote;
    ...
    event VoteAmended(address voterAddress, bytes32 candidateName);
    event VotesTallied();
        ...
        returns (bool)
    {
        ...
        voters.push(Voter({
            ...
            voted: false,
            // Initialise the voters vote as blank
            vote: bytes32(0),
    ...
    /// @notice Assign all the votes cast in the election to the correct candidate
    function tallyVotes() 
        internal
        onlyAdmin
        electionHasEnded
        resultsHaveNotBeenCalculated
    {
        ...
        for (uint i = 0; i < voters.length; i++) {
            ...
            // Assign the voters vote if they have voted
            if (voter.voted) {
                // Retrieve the voters constituency information
                ...
                uint candidateIndex = constituency.candidateIndexs[voter.vote];
